...
/**
对于读写flash而言，读写都需要输入指令+3字节地址（W25Q256地址为4字节，存储越大，地址越大
W25Q128结构

片   16MB    1       0x 000000 ~ 0x FFFFFF
块   64KB    256     0x xx0000 ~ 0x xxFFFF  xx为各个块的地址， 0000~FFFF为块内地址
扇   4KB     4096    0x xxx000 ~ 0x xxxFFF  xxx为各个扇的地址，000~FFF为扇内地址
页   256B    65536   0x xxxx00 ~ 0x xxxxFF  xxxx为各个页的地址，00~FF为页内地址
...
/**
 * @brief    W25QXX擦除一个扇区
 * @param   sector_addr    —— 扇区地址 根据实际容量设置，输入为选中的哪个扇区（0~4095）
 * @retval  none
 * @note    阻塞操作
 */
...
//int W25QXX_Write(uint8_t* dat, uint32_t WriteAddr, uint16_t nbytes)   //GPS数据写入
//{
//    uint16_t page_remain;   //当前页剩余空间
//    uint16_t write_len;     
//    uint32_t current_addr = WriteAddr;
//    uint8_t* current_dat = dat;
//    uint16_t remaining = nbytes;

//    while (remaining > 0) {
//        /* 计算当前页剩余空间 */
//        page_remain = 256 - (current_addr % 256);
//        
//        /* 本次最多写多少 */
//        write_len = (remaining < page_remain) ? remaining : page_remain;
//        
//        /* 单页写入 */
//        W25QXX_Page_Program(current_dat, current_addr, write_len);
//        
//        /* 更新指针和计数 */
//        current_addr += write_len;
//        current_dat += write_len;
//        remaining -= write_len;
//    }
//    
//    return 0;
